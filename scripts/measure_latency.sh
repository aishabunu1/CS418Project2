#!/usr/bin/env bash
# =============================================================================
# measure_latency.sh — Measure glass-to-glass latency for 2, 4, 6s segments
#
# Method: Record a visible timer/clock on camera, stream it, screenshot the
# player, compare timestamps. This script automates starting streams and
# collecting segment delivery timing from MPD updates.
# =============================================================================
...
for SEG_DUR in 2 4 6; do
    ...
    # Start stream with this segment size in background
    "${SCRIPT_DIR}/start_stream.sh" "${SEG_DUR}" &
    STREAM_PID=$!
    sleep $(( SEG_DUR * 3 + 2 ))   # Wait for MPD to appear + buffer
    ...
    # Read the publishTime from the MPD to estimate end-to-end delay
    ...
    # Approx min latency = 1 segment + encoder pipeline delay (~0.5–1s with ultrafast)
